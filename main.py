...
icon = Image.open("C:\ocr\ocr image.png")
st.set_page_config(page_title="BizCardX: Extracting Business Card Data with OCR | By B Praveen kumar",
                   page_icon=icon,
                   layout="wide",
...
def setting_bg():
    ...
    <style>
        .stApp {{
            background: linear-gradient(to right, #2FE3EC, #F06C2E );
...
#mycursor.execute("CREATE DATABASE card_data")

# TABLE CREATION
# mycursor.execute('''CREATE TABLE card_data.card_details
    ...
    if uploaded_card is not None:
        ...
        if st.button("Upload to Database"):
            for i,row in df.iterrows():
                ...
                mycursor.execute('USE card_data')
